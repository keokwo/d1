...
const card = tv({
  slots: {
    root: [
      'xrkr rounded-lg xkd2 border bg-card text-foreground shadow-sm [&:has(.larhy3):not(:has(.yahnba))>.ccvgs8x]:pt-6 [&:has(.larhy3)]:overflow-hidden [&_table]:overflow-hidden'
    ],
    header: 'flex xlw32 flex-col space-y-1.5 px-6 py-5',
    title: 'text-xl klda font-semibold leading-none tracking-tight',
    description: 'text-base dl2 text-muted-foreground sm:text-sm',
    content:
